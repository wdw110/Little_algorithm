...
python实现Mysql数据库连接池
Copyed by 2016年12月5号

@author: wdw110
1. 执行带参数的SQL时，请先用sql语句指定需要输入的条件列表，然后再用tuple/list进行条件匹配
2. 在格式SQL中不需要使用引号指定数据类型，系统会根据输入参数自动识别
3. 在输入的值中不需要使用转义函数，系统会自动处理
'''
...
import Config   #Config是一些数据库的配置文件

class Mysql(object):
	"""
	MYSQL数据库对象，负责产生数据库连接，此类中的连接采用连接池实现获取连接对象：
	conn = Mysql.getConn()释放连接对象；conn.close()或del conn
	"""
	#连接池对象
	__pool = None
	def __init__(self):
		#数据库构造函数，从连接池中取出链接，并生成操作游标
